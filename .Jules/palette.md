...
## 2026-06-12 - Transient Notifications Focus
**Learning:** In CustomTkinter, applying `grab_set()` or `focus_set()` to transient notifications (like auto-dismissing toasts) creates a severe UX regression by interrupting user workflow. Only apply modality and focus-stealing to explicit, blocking dialogs.
**Action:** Remove `grab_set()` from `ToastPopup` so it no longer steals focus and interrupts the user.
