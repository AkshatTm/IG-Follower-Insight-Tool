...
## 2024-06-14 - Non-blocking Transient Notifications
**Learning:** In CustomTkinter, applying `grab_set()` or `focus_set()` to transient notifications (like auto-dismissing toasts) creates a severe UX regression by interrupting user workflow.
**Action:** Only apply modality and focus-stealing to explicit, blocking dialogs. Remove `grab_set()` from `ToastPopup` to allow users to continue interacting with the app while the toast is visible.
