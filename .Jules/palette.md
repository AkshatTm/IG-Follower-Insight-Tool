...
## 2024-06-04 - Transient Notifications Focus
**Learning:** In CustomTkinter, applying `grab_set()` or `focus_set()` to transient notifications (like auto-dismissing toasts) creates a severe UX regression by interrupting user workflow. Modal dialogs trap keyboard-only users if they do not bind dismissal keys.
**Action:** Only apply modality and focus-stealing to explicit, blocking dialogs (e.g. `duration_ms == 0`). Always bind dismissal keys (`<Escape>`, `<Return>`) to the destroy method to ensure keyboard accessibility.
