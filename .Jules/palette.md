...
## 2024-05-18 - Modal Dialog Accessibility and Workflow
**Learning:** In CustomTkinter, applying `grab_set()` unconditionally to transient notifications (like auto-dismissing toasts) creates a severe UX regression by interrupting user workflow. Additionally, explicit modals can trap keyboard-only users without proper bindings.
**Action:** Only apply modality (`grab_set()`) to explicit, blocking dialogs. For these dialogs, always call `self.focus_set()` and bind dismissal keys (`<Escape>`, `<Return>`) to the destroy method to ensure keyboard accessibility.
