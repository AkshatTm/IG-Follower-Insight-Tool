...
## 2024-05-31 - Toast Notification Modality and Keyboard Accessibility
**Learning:** Applying modality (`grab_set()` or `focus_set()`) to transient notifications (like auto-dismissing toasts) creates a severe UX regression by interrupting user workflow. Additionally, modal dialogs (`CTkToplevel`) can trap keyboard-only users without explicit focus and keyboard bindings.
**Action:** Only apply modality and focus-stealing to explicit, blocking dialogs. Always call `self.focus_set()` and bind dismissal keys (`<Escape>`, `<Return>`) to the destroy method to ensure keyboard accessibility.
