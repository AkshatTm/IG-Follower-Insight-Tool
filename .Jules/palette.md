...
## 2024-06-11 - Toast Modality & Accessibility
**Learning:** In CustomTkinter, applying `grab_set()` or `focus_set()` to transient notifications (like auto-dismissing toasts) creates a severe UX regression by interrupting user workflow. Additionally, persistent modal dialogs can trap keyboard-only users if not implemented correctly.
**Action:** Only apply modality and focus-stealing to explicit, blocking dialogs (e.g., when duration <= 0). Always bind dismissal keys (`<Escape>`, `<Return>`) to the destroy method for these blocking dialogs to ensure keyboard accessibility.
