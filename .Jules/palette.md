...
## 2024-05-18 - Non-blocking Transient Toasts
**Learning:** In CustomTkinter, applying `grab_set()` or `focus_set()` to transient notifications (like auto-dismissing toasts) creates a severe UX regression by interrupting user workflow and stealing focus unnecessarily.
**Action:** Only apply modality and focus-stealing to explicit, blocking dialogs (e.g., when `duration_ms == 0`).

## 2024-05-18 - Keyboard Accessible Modal Dialogs
**Learning:** Modal dialogs (`CTkToplevel`) with `grab_set()` can trap keyboard-only users if they don't explicitly handle standard dismissal keys.
**Action:** Always call `self.focus_set()` and bind dismissal keys (`<Escape>`, `<Return>`) to the destroy method for blocking modals.
