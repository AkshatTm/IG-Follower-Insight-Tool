...
## 2024-05-22 - Modal Keyboard Accessibility
**Learning:** In CustomTkinter, modal dialogs (`CTkToplevel`) can trap keyboard-only users if focus and keyboard dismissal bindings are missing.
**Action:** Always call `self.focus_set()` and bind dismissal keys (`<Escape>`, `<Return>`) to the destroy method (e.g., using `lambda e: self.destroy()` to absorb the event argument) to ensure keyboard accessibility.
