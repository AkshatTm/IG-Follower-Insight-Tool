...
**Learning:** In CustomTkinter, modal dialogs (`CTkToplevel`) can trap keyboard-only users because they do not automatically receive focus or provide default keyboard dismissal methods.
**Action:** Always call `self.focus_set()` and bind dismissal keys (`<Escape>`, `<Return>`) to the destroy method to ensure keyboard accessibility.
