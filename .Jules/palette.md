...
## 2024-05-24 - Toast Popup Accessibility
**Learning:** Applying `grab_set()` or `focus_set()` to transient notifications (like auto-dismissing toasts) creates a severe UX regression by interrupting user workflow. Only apply modality and focus-stealing to explicit, blocking dialogs. Additionally, modal dialogs (`CTkToplevel`) can trap keyboard-only users. Always call `self.focus_set()` and bind dismissal keys (`<Escape>`, `<Return>`) to the destroy method to ensure keyboard accessibility.
**Action:** Use conditional logic for `grab_set()` based on duration and bind keys.
