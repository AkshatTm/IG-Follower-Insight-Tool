...
## 2024-05-18 - Keyboard Accessibility for CustomTkinter Modal Dialogs
**Learning:** In CustomTkinter, modal dialogs (`CTkToplevel`) can trap keyboard-only users because they don't automatically grab keyboard focus or support standard dismissal keys.
**Action:** Always call `self.focus_set()` and bind standard dismissal keys (`<Escape>`, `<Return>`) to the destroy method to ensure keyboard accessibility in modal dialogs.
