...
## 2026-07-07 - Add keyboard focus states to CustomTkinter Entries
**Learning:** CustomTkinter `CTkEntry` widgets lack default visual focus states out of the box, making keyboard navigation difficult.
**Action:** Manually bind `<FocusIn>` and `<FocusOut>` events to update visual properties like `border_color`, and bind `<Escape>` to clear text and yield focus using `focus_set()`.
