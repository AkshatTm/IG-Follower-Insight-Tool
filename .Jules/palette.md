...
## 2026-05-21 - Modal Dialog Keyboard Accessibility
**Learning:** In CustomTkinter, modal dialogs (`CTkToplevel`) can trap keyboard-only users because they do not automatically receive focus or map standard dismissal keys.
**Action:** Always call `self.focus_set()` and bind dismissal keys (`<Escape>`, `<Return>`) to the destroy method on `CTkToplevel` components to ensure keyboard accessibility.
