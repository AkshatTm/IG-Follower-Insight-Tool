...
## 2026-07-01 - Prevent Focus Stealing on Transient Toasts
**Learning:** In CustomTkinter, applying `grab_set()` or `focus_set()` to transient notifications (like auto-dismissing toasts) creates a severe UX regression by interrupting user workflow, taking away control from the main window.
**Action:** Only apply modality and focus-stealing to explicit, blocking dialogs. Never apply `grab_set()` to temporary informational popups like Toasts.
