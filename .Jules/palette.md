...
## 2024-05-19 - Keyboard Accessibility for Modals
**Learning:** In CustomTkinter, modal dialogs (`CTkToplevel`) can trap keyboard-only users because they do not automatically focus themselves or bind standard dismissal keys.
**Action:** When creating modals or temporary popups (like `ToastPopup`), always call `self.focus_set()` and bind standard dismissal keys (`<Escape>`, `<Return>`) to the destroy method using `lambda e: self._safe_destroy()` to absorb the event argument, ensuring keyboard accessibility.
