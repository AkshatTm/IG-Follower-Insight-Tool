...
## 2024-06-23 - Transient Toast Modality and Keyboard Accessibility
**Learning:** Applying `grab_set()` to transient (auto-dismissing) notifications interrupts user workflows. Furthermore, blocking dialogs must include explicit keyboard dismissal bindings (`<Escape>`, `<Return>`) and focus management to prevent trapping keyboard-only users.
**Action:** Only apply `grab_set()` and `focus_set()` to explicit, blocking dialogs. Ensure these dialogs always bind standard dismissal keys.
