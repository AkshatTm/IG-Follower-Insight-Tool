...
## 2024-05-19 - Duplicate Render Handling
**Learning:** Having multiple overlapping methods (like duplicate `_populate_rows` definitions) handling UI state and empty-state toggling leads to unpredictable blank screens and visual bugs.
**Action:** Always ensure a single source of truth for list rendering and empty state fallback toggles.
