...
## 2024-05-24 - Transient Notifications Stealing Focus
**Learning:** In CustomTkinter, applying `grab_set()` (modal dialog behavior) to temporary auto-dismissing toast notifications prevents users from continuing their workflow (like typing or clicking elsewhere) while the toast is visible. This creates a highly frustrating "stuck" feeling.
**Action:** Only apply `grab_set()` and `focus_set()` to explicit, user-blocking dialogs. For transient notifications that dismiss themselves, avoid modality so the user can continue working uninterrupted.
