...
## 2024-05-19 - Keyboard Accessibility for Modal Popups
**Learning:** In CustomTkinter, modal dialogs (`CTkToplevel`) can trap keyboard-only users. Setting focus automatically is not guaranteed, meaning users cannot dismiss dialogs using the keyboard, frustrating users and violating accessibility guidelines.
**Action:** Always call `self.focus_set()` and bind dismissal keys (`<Escape>`, `<Return>`) to the destroy method to ensure keyboard accessibility in modal dialogs.
