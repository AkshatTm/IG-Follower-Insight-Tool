...
## 2026-06-27 - Toast Popup Accessibility
**Learning:** In CustomTkinter, applying `grab_set()` to transient notifications interrupts user workflow. Also, modal dialogs need keyboard bindings like `<Escape>` and `<Return>` to prevent trapping keyboard-only users.
**Action:** Only apply `grab_set()` and `focus_set()` to explicit, blocking dialogs (e.g., when duration_ms <= 0), and bind dismissal keys to the destroy method to ensure keyboard accessibility.
