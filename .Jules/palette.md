...
## 2024-06-08 - Non-blocking Toast Notifications
**Learning:** In CustomTkinter, applying `grab_set()` to transient notifications (like auto-dismissing toasts) creates a severe UX regression by interrupting user workflow. Only apply modality and focus-stealing to explicit, blocking dialogs.
**Action:** Remove `grab_set()` from `ToastPopup` to ensure notifications are non-blocking.
