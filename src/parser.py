...
def parse_instagram_json(filepath: str) -> Set[str]:
    ...
    # Pattern A (current): [ {"string_list_data": [{"value": "user"}]} ]
    ...
    """
    # Prevent Memory Exhaustion (DoS): Check if file is excessively large
    # before parsing. Parsing very large JSON files can crash the app.
    max_size_bytes = 100 * 1024 * 1024

    # Explicitly verify it's a regular file first, as os.path.getsize()
    # returns 0 for char devices like /dev/zero, outputting infinite data.
    if not os.path.isfile(filepath):
        raise ValueError("Invalid file path: Must be a regular file.")
...
def _extract_username(entry: dict) -> str | None:
    ...
    # Limit username length to 100 chars to prevent artificially
    # massive strings from freezing the UI (DoS mitigation).
    max_len = 100
...
def calculate_non_followers(
    following_set: Set[str], followers_set: Set[str]
) -> List[str]:
    """
