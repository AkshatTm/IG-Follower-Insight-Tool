...
class ScreenFilter(ctk.CTkFrame):
    ...
    def __init__(self, master, app):
        ...
        # Track rendered row widgets via a reusable pool
        self._row_pool = []
    ...
    def _create_row_widget(self):
        """Create a single generic row widget for the pool."""
        row = ctk.CTkFrame(
            self.scroll_frame,
            fg_color="transparent",
            height=40,
    ...
    def _populate_rows(self, clear=False):
        ...
        # Count how many are currently packed
        start_idx = sum(1 for item in self._row_pool if item["is_packed"])
        end_idx = min(self._visible_limit, len(self._filtered_users))

        # Grow the pool if needed
        while len(self._row_pool) < end_idx:
        ...
        for i in range(start_idx, end_idx):
            ...
            # Update data bindings if this widget is representing a different user
