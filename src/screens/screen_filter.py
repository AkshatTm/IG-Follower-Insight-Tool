...
class ScreenFilter(ctk.CTkFrame):
    ...
    def __init__(self, master, app):
        ...
        # Widget pool for reusing list items: [{"row", "user_label", "switch", "is_packed"}]
        self._row_pool = []
    ...
    def _populate_rows(self, clear=False):
        """Create or reuse rows for the currently visible slice of filtered users with VIP toggle switches."""
        if getattr(self, '_load_more_btn', None) is not None and self._load_more_btn.winfo_exists():
            self._load_more_btn.destroy()

        start_idx = 0 if clear else getattr(self, '_visible_count', 0)
        ...
        # ⚡ OPTIMIZATION: Implement widget pooling with pack_forget() instead of destroy()
        # This prevents severe UI thread blocking during rapid search/filtering by reusing existing widgets.
        for i in range(start_idx, end_idx):
            ...
            if i < len(self._row_pool):
                ...
                item["user_label"].configure(text=f"@{username}")
                item["switch"].configure(variable=var)
            ...
            handler = create_toggle_handler(username)
            item["switch"].configure(command=handler)
            item["row"].bind("<Button-1>", handler)
            if hasattr(item["row"], "_canvas"):
                item["row"]._canvas.bind("<Button-1>", handler)
            item["user_label"].bind("<Button-1>", handler)
            if hasattr(item["user_label"], "_label"):
                item["user_label"]._label.bind("<Button-1>", handler)

        if clear:
            for i in range(end_idx, len(self._row_pool)):
                item = self._row_pool[i]
                if item["is_packed"]:
                    item["row"].pack_forget()
                    item["is_packed"] = False

        self._visible_count = end_idx
