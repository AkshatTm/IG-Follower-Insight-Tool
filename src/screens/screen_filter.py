...
class ScreenFilter(ctk.CTkFrame):
    ...
    def _populate_rows(self, clear=False):
        """Create rows for the currently visible slice of filtered users using a widget pool."""
        if clear:
            self._rendered_count = 0
        ...
        for i in range(start_idx, end_idx):
            ...
            pool_item["user_label"].configure(text=f"@{username}")

            # Update state when switch is clicked directly
            handler = create_toggle_handler(username, var)
            pool_item["switch"].configure(variable=var, command=handler)

            # Bind click events for row-level toggling
            pool_item["frame"].bind("<Button-1>", handler)
            if hasattr(pool_item["frame"], "_canvas"):
                pool_item["frame"]._canvas.bind("<Button-1>", handler)
