...
class ScreenFilter(ctk.CTkFrame):
    ...
    def _populate_rows(self, clear=False):
        ...
        # [PERFORMANCE]: Widget Pooling
        # Instead of constantly destroying and creating Tkinter widgets during search,
        # we pre-initialize and reuse a generic pool of row widgets via pack_forget().
        # This dramatically reduces layout thrashing and prevents UI thread blocking.
        if clear:
        ...
        for i in range(start_idx, end_idx):
            ...
            if i < len(self._row_pool):
                # Reuse existing widget from the pool
                ...
                switch.configure(variable=var, command=handler)

                # Rebind click events for row-level toggling to the new handler
