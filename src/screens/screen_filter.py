...
class ScreenFilter(ctk.CTkFrame):
    ...
    def __init__(self, master, app):
        ...
        # Source of truth for VIP state (plain dict, lazy eval for BooleanVars)
        self._vip_state = {
            u: (u in self.whitelist)
            for u in self.non_followers
        }
        # Keep track of active BooleanVars
        self._vip_vars = {}
    ...
    def _build_header(self, parent):
        ...
        self.subtitle = ctk.CTkLabel(
            header,
            text=f"Showing {len(self.non_followers)} users who don't follow you back",  # noqa: E501
            font=Fonts.BODY,
    ...
    def _build_user_list(self, parent):
        ...
        header_frame.pack(
            fill="x", padx=Spacing.CARD_PAD_X, pady=(
    ...
    def _populate_rows(self, clear=False):
        """Create a row for each non-follower with a VIP toggle switch. Uses pagination."""  # noqa: E501
        if clear:
            for widget in self._row_widgets.values():
                widget.destroy()
            self._row_widgets.clear()
            self._vip_vars.clear()
        ...
        for i in range(start_idx, end_idx):
            ...
            # Lazily instantiate the BooleanVar for this row
            if username not in self._vip_vars:
                self._vip_vars[username] = ctk.BooleanVar(
                    value=self._vip_state[username])
            ...
            switch = ctk.CTkSwitch(
                ...
                command=lambda u=username, v=var: (
                    self._vip_state.update({u: v.get()}),
                    self._update_counter()
                )
            )
    ...
    def _perform_search(self):
        ...
        if visible_count == 0:
            if len(self.non_followers) == 0:
                self.empty_state_label.configure(
                    text="No non-followers to filter! 🎉")
            else:
                self.empty_state_label.configure(
        ...
        if query:
            self.subtitle.configure(
                text=f"Showing {len(self._filtered_users)} of {len(self.non_followers)} users (filtered)"  # noqa: E501
            )
        else:
            self.subtitle.configure(
                text=f"Showing {len(self.non_followers)} users who don't follow you back"  # noqa: E501
