...
class ScreenFilter(ctk.CTkFrame):
    ...
    def __init__(self, master, app):
        ...
        # True widget pooling for pagination and search
        self._row_pool = []
    ...
    def _populate_rows(self, clear=False):
        ...
        start_idx = 0 if clear else sum(1 for item in self._row_pool if item["is_packed"])
        ...
        for i in range(start_idx, end_idx):
            ...
            if i < len(self._row_pool):
                ...
                row, user_label, switch, var = pool_item["widgets"]
                pool_item["username"] = username
            else:
                var = ctk.BooleanVar()
                row = ctk.CTkFrame(
                    self.scroll_frame,
                ...
                pool_item = {
                    "widgets": (row, user_label, switch, var),
                    "username": username,
                    "is_packed": False
                }
            ...
            var.set(self._vip_state.get(username, False))
        ...
        if clear:
            for i in range(end_idx, len(self._row_pool)):
                if self._row_pool[i]["is_packed"]:
                    self._row_pool[i]["widgets"][0].pack_forget()
                    self._row_pool[i]["is_packed"] = False
    ...
    def _toggle_select_all(self):
        ...
        # Update UI for currently visible widgets
        for item in self._row_pool:
            if item["is_packed"] and item["username"] in self._filtered_users:
                item["widgets"][3].set(self._all_selected)
