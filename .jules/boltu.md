...
## 2024-06-16 - Pre-compiling Regex in Hot Paths
**Learning:** When performing regex substitutions inside high-volume operations like processing large JSON arrays, using `re.sub()` directly repeatedly recompiles the pattern. Explicitly pre-compiling the regex at the module level using `re.compile()` reduces parsing and lookup overhead.
**Action:** Always pre-compile regex patterns that are used inside high-volume loops.
