...
## 2026-07-05 - CustomTkinter Widget Pooling
**Learning:** Destroying CustomTkinter widgets causes severe lag (O(N) operations blocking the main thread), and instantiating widgets is expensive. Storing widgets in a dictionary by ID causes memory leaks when searching.
**Action:** Use a widget pooling pattern: maintain a list of instantiated generic widgets, configure their values based on visible indices dynamically, and use `pack_forget` to hide unused ones during search. Add an `is_packed` flag to avoid redundant layout recalculations.
