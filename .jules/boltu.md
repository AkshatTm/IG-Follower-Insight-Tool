...
## 2024-06-01 - [Widget Pooling & Pagination Indexing in CustomTkinter]
**Learning:** When refactoring CustomTkinter lists to use widget pooling (`pack_forget` instead of `destroy`) to prevent UI blocking during frequent search filtering, the length of the widget dictionary (`len(self._row_widgets)`) can no longer be used to calculate pagination offsets (`start_idx`) since the widgets persist as a cache. This causes pagination drift.
**Action:** Explicitly derive pagination indices from the visible limit and page size (`max(0, self._visible_limit - self.PAGE_SIZE)`) to preserve consistent rendering when reusing pooled widgets.
