...
## 2026-06-27 - Widget Pooling in CustomTkinter
**Learning:** In CustomTkinter, implementing 'widget pooling' by caching widgets in an unbounded dictionary indexed by entity ID creates a memory leak and O(N) lag on clearing because destroying widgets blocks the main thread.
**Action:** Instead, use a true pool pattern with a fixed-size list (e.g., self._row_pool) of generic widgets that dynamically update their internal state and bindings based on the visible index, and use .pack_forget() for unused ones.
