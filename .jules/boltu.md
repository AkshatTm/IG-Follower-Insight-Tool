...
## 2026-07-04 - Widget Pooling for Search Filtering
**Learning:** Destroying and recreating CustomTkinter widgets during rapid updates (like search) causes severe UI main thread blocking. Caching widgets in an unbounded dictionary causes memory leaks.
**Action:** Implemented a fixed-size widget pool using a list and `pack_forget()` to reuse widgets and avoid O(N) allocation lag.
