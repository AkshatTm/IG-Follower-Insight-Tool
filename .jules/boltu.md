...
## 2024-05-28 - True Widget Pooling in CustomTkinter
**Learning:** In CustomTkinter, implementing "widget pooling" by caching widgets in an unbounded dictionary indexed by entity ID creates a memory leak and O(N) lag on clearing. Indiscriminately destroying and recreating widgets on every keystroke causes severe UI thread blocking.
**Action:** Use a true pool pattern with a fixed-size list (`self._row_pool`) of generic widgets that dynamically update their internal state and bindings based on the visible index, and use `pack_forget()` instead of `destroy()`.
