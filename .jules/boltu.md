...
## 2026-06-03 - Tkinter Widget Pooling in List Views
**Learning:** In CustomTkinter lists with frequent updates (like search filtering), indiscriminately destroying and recreating thousands of widgets blocks the main UI thread severely.
**Action:** Pre-compute searchable data and use `pack_forget()` instead of `destroy()` to create a widget pool. Reuse existing widgets from the pool and manually track visible widget counts for pagination logic.
