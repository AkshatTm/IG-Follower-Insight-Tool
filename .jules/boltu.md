...
## 2024-05-28 - Regex pre-compilation for parsing optimization
**Learning:** Using `re.sub` directly inside an extraction loop for every array element incurs repeated parsing and lookup overhead for the regex pattern.
**Action:** When applying regular expressions repetitively inside large data processing operations, explicitly pre-compile them at the module level using `re.compile()` to boost performance (~2x faster).
