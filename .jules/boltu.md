...
## 2024-05-28 - Regex Compilation in High-Volume Operations
**Learning:** Parsing and compiling regex patterns inline using `re.sub(pattern, ...)` inside a loop iterating over large data structures (like Instagram export JSON lists) incurs significant overhead, as Python must perform a cache lookup on every iteration.
**Action:** Always explicitly pre-compile regular expressions at the module level using `re.compile()` and call `.sub()` on the compiled pattern directly when processing high-volume data arrays to avoid lookup overhead.
