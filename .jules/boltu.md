...
## 2024-04-29 - Dictionary parsing optimization
**Learning:** In Python, handling many missing keys using `try/except` and `.get()` blocks within loops over large JSON arrays is slower compared to directly checking for key existence via `in`. Avoid exception handling for normal control flows that occur frequently.
**Action:** Use direct key existence checks (`if 'key' in dict:`) instead of `try/except` blocks for dictionary lookups.
