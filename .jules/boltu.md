...
## 2024-05-29 - [True Widget Pooling Pattern in CustomTkinter]
**Learning:** Implementing "widget pooling" by caching widgets in a dictionary indexed by username creates an unbounded memory leak and O(N) lag on clearing. A true pool pattern must use a fixed-size list of generic row widgets that dynamically update their internal state and callbacks based on their visible index.
**Action:** Use an index-based list (`self._row_pool`) to reuse generic widgets instead of caching widgets per-entity.
