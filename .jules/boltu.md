...
**Learning:** When performing regex substitutions inside high-volume operations (e.g., processing large arrays), explicitly pre-compile the regex at the module level using `re.compile()` to reduce parsing and lookup overhead.
**Action:** Always pre-compile regex patterns at the module level instead of calling `re.sub` inline inside loops or frequent function calls.
